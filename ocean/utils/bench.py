...
import ocean
from ocean.exceptions import PatchError, ValidationError
from ocean.utils import exec_cmd, get_ocean_name, get_cmd_output, log, which

logger = logging.getLogger(ocean.PROJECT_NAME)


@lru_cache(maxsize=None)
def get_env_cmd(cmd: str, ocean_path: str = ".") -> str:
	...
	existing_python_bins = glob(
		os.path.join(ocean_path, "env", "**", "bin", cmd), recursive=True
	)
	...
	cmd = cmd.strip("*")
	return os.path.abspath(os.path.join(ocean_path, "env", "bin", cmd))
...
def update_node_packages(ocean_path=".", apps=None, verbose=None):
	print("Updating node packages...")
	from distutils.version import LooseVersion

	from ocean.utils.app import get_develop_version

	v = LooseVersion(get_develop_version("frappe", ocean_path=ocean_path))

	# After rollup was merged, frappe_version = 10.1
	# if develop_verion is 11 and up, only then install yarn
	if v < LooseVersion("11.x.x-develop"):
		update_npm_packages(ocean_path, apps=apps, verbose=verbose)
	else:
		update_yarn_packages(ocean_path, apps=apps, verbose=verbose)


def install_python_dev_dependencies(ocean_path=".", apps=None, verbose=False):
	import ocean.cli
	...
	if isinstance(apps, str):
		...
		apps = ocean.get_installed_apps()

	for app in apps:
		pyproject_deps = None
		app_path = os.path.join(ocean_path, "apps", app)
		...
		if os.path.exists(pyproject_path):
			...
			if pyproject_deps:
				ocean.run(f"{ocean.python} -m pip install {quiet_flag} --upgrade {pyproject_deps}")

		if not pyproject_deps and os.path.exists(dev_requirements_path):
			ocean.run(
				f"{ocean.python} -m pip install {quiet_flag} --upgrade -r {dev_requirements_path}"
			)
...
def _generate_dev_deps_pattern(pyproject_path):
	...
	with contextlib.suppress(KeyError):
		for pkg, version in pyroject_config["tool"]["ocean"]["dev-dependencies"].items():
			op = "==" if "=" not in version else ""
			requirements_pattern += f"{pkg}{op}{version} "
	return requirements_pattern


def update_yarn_packages(ocean_path=".", apps=None, verbose=None):
	...
	apps_dir = os.path.join(ocean.name, "apps")
...
def update_npm_packages(ocean_path=".", apps=None, verbose=None):
	verbose = ocean.cli.verbose or verbose
	...
	with open(os.path.join(ocean_path, "package.json"), "w") as f:
		f.write(json.dumps(package_json, indent=1, sort_keys=True))

	exec_cmd(npm_install, cwd=ocean_path)
...
def validate_upgrade(from_ver, to_ver, ocean_path="."):
	if to_ver >= 6 and not which("npm") and not which("node") and not which("nodejs"):
		raise Exception("Please install nodejs and npm")


def post_upgrade(from_ver, to_ver, ocean_path="."):
	from ocean.ocean import Ocean
	from ocean.config import redis
	from ocean.config.nginx import make_nginx_conf
	from ocean.config.supervisor import generate_supervisor_config

	conf = Ocean(ocean_path).conf
	print("-" * 80 + f"Your ocean was upgraded to version {to_ver}")

	if conf.get("restart_supervisor_on_update"):
		redis.generate_config(ocean_path=ocean_path)
		generate_supervisor_config(ocean_path=ocean_path)
		make_nginx_conf(ocean_path=ocean_path)
		print(
			"As you have setup your ocean for production, you will have to reload"
...
def patch_sites(ocean_path="."):
	from ocean.ocean import Ocean
	from ocean.utils.system import migrate_site

	ocean = Ocean(ocean_path)

	for site in ocean.sites:
		try:
			migrate_site(site, ocean_path=ocean_path)
		except subprocess.CalledProcessError:
			raise PatchError


def restart_supervisor_processes(ocean_path=".", web_workers=False, _raise=False):
	from ocean.ocean import Ocean
	...
	if cmd:
		ocean.run(cmd, _raise=_raise)

	else:
		sudo = ""
		try:
			supervisor_status = get_cmd_output("supervisorctl status", cwd=ocean_path)
		except subprocess.CalledProcessError as e:
			if e.returncode == 127:
				log("restart failed: Couldn't find supervisorctl in PATH", level=3)
				return
			sudo = "sudo "
			supervisor_status = get_cmd_output("sudo supervisorctl status", cwd=ocean_path)
		...
		if web_workers and f"{ocean_name}-web:" in supervisor_status:
			group = f"{ocean_name}-web:\t"

		elif f"{ocean_name}-workers:" in supervisor_status:
			group = f"{ocean_name}-workers: {ocean_name}-web:"

		# backward compatibility
		elif f"{ocean_name}-processes:" in supervisor_status:
			group = f"{ocean_name}-processes:"

		# backward compatibility
		else:
			group = "frappe:"

		failure = ocean.run(f"{sudo}supervisorctl restart {group}", _raise=_raise)
		if failure:
			log("restarting supervisor failed. Use `ocean restart` to retry.", level=3)


def restart_systemd_processes(ocean_path=".", web_workers=False, _raise=True):
	ocean_name = get_ocean_name(ocean_path)
	exec_cmd(
		f"sudo systemctl stop -- $(systemctl show -p Requires {ocean_name}.target | cut"
		" -d= -f2)",
		_raise=_raise,
	)
	exec_cmd(
		f"sudo systemctl start -- $(systemctl show -p Requires {ocean_name}.target |"
		" cut -d= -f2)",
		_raise=_raise,
	)


def restart_process_manager(ocean_path=".", web_workers=False):
	# only overmind has the restart feature, not sure other supported procmans do
	if which("overmind") and os.path.exists(os.path.join(ocean_path, ".overmind.sock")):
		worker = "web" if web_workers else ""
		exec_cmd(f"overmind restart {worker}", cwd=ocean_path)


def build_assets(ocean_path=".", app=None):
	command = "ocean build"
	if app:
		command += f" --app {app}"
	exec_cmd(command, cwd=ocean_path, env={"BENCH_DEVELOPER": "1"})


def handle_version_upgrade(version_upgrade, ocean_path, force, reset, conf):
	from ocean.utils import log, pause_exec
...
This would take significant time to migrate and might break custom apps."""
	...
	if not reset and conf.get("shallow_clone"):
		log(
			"""shallow_clone is set in your ocean config.
However without passing the --reset flag, your repositories will be unshallowed.
To avoid this, cancel this operation and run `ocean update --reset`.
...
To avoid seeing this warning, set shallow_clone to false in your common_site_config.json
	...
	if version_upgrade[0] or (not version_upgrade[0] and force):
		validate_upgrade(version_upgrade[1], version_upgrade[2], ocean_path=ocean_path)
...
def update(
	...
	"""command: ocean update"""
	import re

	from ocean import patches
	from ocean.app import pull_apps
	from ocean.ocean import Ocean
	from ocean.config.common_site_config import update_config
	from ocean.exceptions import CannotUpdateReleaseOcean
	from ocean.utils.app import is_version_upgrade
	from ocean.utils.system import backup_all_sites

	ocean_path = os.path.abspath(".")
	ocean = Ocean(ocean_path)
	patches.run(ocean_path=ocean_path)
	conf = ocean.conf

	if conf.get("release_ocean"):
		raise CannotUpdateReleaseOcean("Release ocean detected, cannot update!")
	...
	handle_version_upgrade(version_upgrade, ocean_path, force, reset, conf)

	conf.update({"maintenance_mode": 1, "pause_scheduler": 1})
	update_config(conf, ocean_path=ocean_path)

	if backup:
		print("Backing up sites...")
		backup_all_sites(ocean_path=ocean_path)

	if pull:
		print("Updating apps source...")
		pull_apps(apps=apps, ocean_path=ocean_path, reset=reset)

	if requirements:
		print("Setting up requirements...")
		ocean.setup.requirements()

	if patch:
		print("Patching sites...")
		patch_sites(ocean_path=ocean_path)
	...
	if version_upgrade[0] or (not version_upgrade[0] and force):
		post_upgrade(version_upgrade[1], version_upgrade[2], ocean_path=ocean_path)

	ocean.reload(web=False, supervisor=restart_supervisor, systemd=restart_systemd)

	conf.update({"maintenance_mode": 0, "pause_scheduler": 0})
	update_config(conf, ocean_path=ocean_path)

	print(
		"_" * 80 + "\nOcean: Deployment tool for Frappe and Frappe Applications"
		" (https://frappe.io/ocean).\nOpen source depends on your contributions, so do"
		" give back by submitting bug reports, patches and fixes and be a part of the"
		" community :)"
	)


def clone_apps_from(ocean_path, clone_from, update_app=True):
	from ocean.app import install_app

	print(f"Copying apps from {clone_from}...")
	subprocess.check_output(["cp", "-R", os.path.join(clone_from, "apps"), ocean_path])

	node_modules_path = os.path.join(clone_from, "node_modules")
	if os.path.exists(node_modules_path):
		print(f"Copying node_modules from {clone_from}...")
		subprocess.check_output(["cp", "-R", node_modules_path, ocean_path])
	...
	def setup_app(app):
		...
		install_app(app, ocean_path, restart_ocean=False)
...
def remove_backups_crontab(ocean_path="."):
	...
	user = Ocean(ocean_dir).conf.get("frappe_user")
...
def set_mariadb_host(host, ocean_path="."):
	update_common_site_config({"db_host": host}, ocean_path=ocean_path)


def set_redis_cache_host(host, ocean_path="."):
	update_common_site_config({"redis_cache": f"redis://{host}"}, ocean_path=ocean_path)


def set_redis_queue_host(host, ocean_path="."):
	update_common_site_config({"redis_queue": f"redis://{host}"}, ocean_path=ocean_path)


def set_redis_socketio_host(host, ocean_path="."):
	update_common_site_config({"redis_socketio": f"redis://{host}"}, ocean_path=ocean_path)


def update_common_site_config(ddict, ocean_path="."):
	filename = os.path.join(ocean_path, "sites", "common_site_config.json")
...
def validate_app_installed_on_sites(app, ocean_path="."):
	print("Checking if app installed on active sites...")
	ret = check_app_installed(app, ocean_path=ocean_path)

	if ret is None:
		check_app_installed_legacy(app, ocean_path=ocean_path)
	else:
		return ret


def check_app_installed(app, ocean_path="."):
	try:
		out = subprocess.check_output(
			["ocean", "--site", "all", "list-apps", "--format", "json"],
...
def check_app_installed_legacy(app, ocean_path="."):
	site_path = os.path.join(ocean_path, "sites")

	for site in os.listdir(site_path):
		req_file = os.path.join(site_path, site, "site_config.json")
		if os.path.exists(req_file):
			out = subprocess.check_output(
				["ocean", "--site", site, "list-apps"], cwd=ocean_path
			).decode("utf-8")
...
def validate_branch():
	from ocean.ocean import Ocean
	from ocean.utils.app import get_current_branch
...
To switch to your required branch, run the following commands: ocean switch-to-branch [branch-name]"""
			)
